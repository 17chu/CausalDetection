...
#triple quotes were commented out for causal sentence extraction by chris for unlabled data
#this is for the purpose of picking up causal cues
...
def PipeLineTest(line):
    ...
    #editied by chris to get sentence and non legitimate val, unable to use parseLine for naked data
...
cue_file = "CausalCues_WithoutModifiers_WithoutComments.txt" #replace with verb file
...
semEvalData_file = "abcnews-date-text.txt" #data file
readData = open(semEvalData_file,'r')
#added by chris
nfile = open('abcnews-date-testCausalSent.txt','w')
sys.exit()
...
type_2 = 0
ans = None
#sys.exit("") #temporarily deleted
for line in readData:
    """sentence,val = parseLine(line)"""
    sentence = line
    #Chris notes: Ans will be a causal cue if the sentencec is causal
    ans = analyze(sentence)
    choice = 0
    if (ans != None):
        choice = 1
        nfile.write(sentence) 
"""    
    if (val == choice):
        #print("CORRECT:")
        #if (val == 0):
